...
if (!hasSupabaseEnv) {
  describe('Create/Read/Update/Delete (deliveries)', () => {
...
describe('Supabase CRUD integration (deliveries)', () => {
  ...
  describe('Create', () => {
    ...
    it('deliveries: create -> store payload for Read/Update/Delete', async () => {
      ...
      ids = { branchId, staffId, salesId, deliveryId }
  ...
  describe('Read', () => {
    it('deliveries: read -> fetch created entity', async () => {
      const fetched = await getDeliveryById(client, ids.deliveryId)
  ...
  describe('Update', () => {
    it('deliveries: update -> apply changes', async () => {
      const updated = await updateDelivery(client, ids.deliveryId, { status: 'Delivered' })
  ...
  describe('Delete', () => {
    it('deliveries: delete -> remove entity + verify non-existent', async () => {
      await deleteDelivery(client, ids.deliveryId)

      await expect(getDeliveryById(client, ids.deliveryId)).resolves.toBeNull()
      await expect(deleteDelivery(client, ids.deliveryId)).rejects.toThrow(/not found/i)
